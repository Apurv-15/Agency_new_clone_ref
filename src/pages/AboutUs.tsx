            ...
            <div className="min-h-screen text-[#1D1D1F] font-sans selection:bg-blue-500/30">
                ...
                <main className="relative z-10 pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto flex flex-col gap-8">
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-auto">
                        {/* Main Hero Card */}
                        <Card3D className="lg:col-span-2 p-8 md:p-12 flex flex-col justify-between min-h-[420px] bg-white/80 backdrop-blur-md hover:bg-[#FDF2FF]/90 transition-colors relative overflow-hidden group/hero">
                            ...
                            <div className="relative z-10">
                                ...
                                <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6 leading-[1.1] text-[#1D1D1F]">
                                    We Are <br />
                            ...
                            <div className="space-y-6 relative z-10">
                                <p className="text-xl md:text-2xl font-medium text-[#424245] max-w-xl leading-snug">
                                    An <span className="text-[#1D1D1F] font-semibold">ISO 9001:2000</span> certified powerhouse.
                        ...
                        {/* Stats / ISO Card */}
                        <Card3D delay={0.1} className="p-8 flex flex-col justify-between bg-gradient-to-br from-[#1D1D1F] to-[#010101] text-white">
                            <div className="space-y-4">
                                <div className="w-12 h-12 rounded-xl bg-white/10 backdrop-blur-xl flex items-center justify-center mb-6">
                                    <ShieldCheck className="w-6 h-6 text-white" />
                                </div>
                                <h3 className="text-2xl font-semibold leading-tight">Certified<br />Excellence</h3>
                            </div>
                            <div className="mt-12 relative">
                                <div className="text-[10px] font-bold opacity-60 mb-1 uppercase tracking-widest">Certification ID</div>
                                <div className="text-4xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">9001:2000</div>
                            ...
                            {activities.map((act, i) => (
                                <Card3D key={i} delay={0.2 + (i * 0.1)} className="p-1 group cursor-pointer hover:!scale-[1.03] hover:shadow-violet-500/10 transition-all">
                                    <div className="bg-white group-hover:bg-[#F5F3FF] rounded-[1.8rem] p-5 flex items-center gap-5 h-full relative z-10 transition-colors">
                                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-tr ${act.gradient} flex items-center justify-center shadow-lg transform group-hover:rotate-6 transition-transform duration-300`}>
                                            {act.icon}
                                        </div>
                                        <h3 className="font-semibold text-lg text-[#1D1D1F] group-hover:text-violet-700 transition-colors">{act.title}</h3>
                                    </div>
                                </Card3D>
                            ))}
                        </div>
                                ...
                                {team.map((member, i) => (
                                    <Card3D key={i} delay={0.3 + (i * 0.1)} className="p-8 flex flex-col justify-center group bg-white hover:bg-[#FDF2FF]">
                                        <div className="flex items-start gap-4 mb-6">
                                            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 border border-white shadow-inner flex items-center justify-center text-lg font-bold text-gray-400 group-hover:text-violet-600 group-hover:scale-110 transition-all duration-300">
                                                {member.name.charAt(0)}
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-lg text-[#1D1D1F] leading-tight group-hover:text-violet-900 transition-colors">{member.name}</h3>
                                                <p className="text-[11px] font-bold text-violet-600 uppercase tracking-widest mt-1">{member.role}</p>
                                            </div>
                                        </div>
                                        <p className="text-[15px] text-[#86868b] leading-relaxed font-medium group-hover:text-[#424245] transition-colors">
                                            &quot;{member.desc}&quot;
                                        </p>
                                    </Card3D>
                                ))}
